...
export default function InventoryPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: (item: InventarioItem) => (
        <div className="flex flex-col">
          <span className={`text-xs font-black ${item.cantidad <= item.stock_minimo ? 'text-error' : 'text-primary'}`}>{item.cantidad.toLocaleString()}</span>
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-4 animate-fade-in-up px-2 md:px-0">
      ...
      <Dialog isOpen={isMovementModalOpen} onOpenChange={setIsMovementModalOpen} title="Entrada / Salida de Stock"><form onSubmit={handleSubmitMove(handleCreateMovement)} className="space-y-6 pt-2"><div className="p-4 rounded-2xl bg-surface-container border border-outline-variant/30 flex items-center justify-between"><div><span className="text-[10px] font-black text-primary/60 uppercase">Existencia</span><p className="text-2xl font-black text-primary leading-none mt-1">{selectedItem?.cantidad} <span className="text-xs text-outline">{selectedItem?.unidad_medida}</span></p></div><div className={`px-4 py-1.5 rounded-full text-[10px] font-black border ${selectedItem?.cantidad <= (selectedItem?.stock_minimo || 0) ? 'bg-error-container/20 text-error border-error' : 'bg-green-50 text-green-700 border-green-200'}`}>{selectedItem?.cantidad <= (selectedItem?.stock_minimo || 0) ? 'REABASTECER' : 'SUFICIENTE'}</div></div><div className="grid grid-cols-2 gap-4"><div className="col-span-2 flex p-1 bg-surface-container-high rounded-xl"><button type="button" onClick={() => setMoveValue("tipo", "ENTRADA")} className={`flex-1 py-3 rounded-lg font-black text-xs transition-all ${moveType === "ENTRADA" ? "bg-green-600 text-white shadow-md" : "text-outline hover:bg-white/50"}`}>ENTRADA</button><button type="button" onClick={() => setMoveValue("tipo", "SALIDA")} className={`flex-1 py-3 rounded-lg font-black text-xs transition-all ${moveType === "SALIDA" ? "bg-error text-white shadow-md" : "text-outline hover:bg-white/50"}`}>SALIDA</button></div><Input label="Cantidad" type="number" step="0.01" required {...registerMove("cantidad", { valueAsNumber: true })} error={errorsMove.cantidad?.message} /><Input label="Motivo" required placeholder="Ej: Compra, Venta, Merma..." {...registerMove("motivo")} error={errorsMove.motivo?.message} /></div><button type="submit" className={`w-full h-12 rounded-xl font-bold text-white shadow-lg transition-all ${moveType === "ENTRADA" ? "bg-green-600 shadow-green-200" : "bg-error shadow-error/20"}`}>CONFIRMAR {moveType}</button></form></Dialog>
